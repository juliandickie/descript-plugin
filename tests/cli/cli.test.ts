...
test("transcript rejects a valueless --timecodes-every without calling the API", async () => {
  ...
  installNoNetwork();
  const c = capture();
  const code = await runCli(["transcript", "p1", "--format", "txt", "--timecodes-every"],
    { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
  assert.equal(code, 2);
...
test("COMMAND_FLAGS lists every flag the registry source reads, per command", async () => {
  ...
  for (let i = 1; i < parts.length; i += 2) {
    ...
    for (const m of block.matchAll(/flags\.([a-zA-Z]+)/g)) read.add(m[1]!);
    for (const m of block.matchAll(/flags\["([a-z-]+)"\]/g)) read.add(m[1]!);
    for (const m of block.matchAll(/badEnum\(ctx, "([a-z-]+)"/g)) read.add(m[1]!);
    if (block.includes("noWait(ctx)")) read.add("no-wait");
    for (const f of read) {
