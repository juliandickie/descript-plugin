...
test("retries a transient 5xx getJob error then succeeds", async () => {
  let i = 0;
  const get = async (): Promise<JobStatus> => {
    i += 1;
    if (i <= 2) throw new DescriptApiError(503, { error: "server_error", message: "boom" });
...
test("does not retry a non-transient getJob error (404)", async () => {
  ...
  await assert.rejects(
    () => pollJob(get, "j", { intervalMs: 1, sleep: async () => {} }),
    (e: unknown) => e instanceof DescriptApiError && e.status === 404
  );
  assert.equal(i, 1);
});
...
test("gives up after maxPollRetries on a persistent transient error", async () => {
  ...
  await assert.rejects(
    () => pollJob(get, "j", { intervalMs: 1, maxPollRetries: 2, sleep: async () => {} }),
    (e: unknown) => e instanceof DescriptApiError && e.status === 503
  );
  assert.equal(i, 3);
});
