...
import {
  reconstructResumeItems,
  validateRequestedFormatsAgainstReport,
  buildResumeReport
} from "../../src/workflows/exportResume.js";
import type { ExportBatchReport, ExportBatchReportItem } from "../../src/workflows/exportBatch.js";
import type { ExportFormat } from "../../src/workflows/exportPublished.js";
...
test("validateRequestedFormatsAgainstReport rejects disjoint format set", () => {
  ...
  if (!v.ok) assert.match(v.reason, /no items in the report attempted/);
});
...
test("buildResumeReport combines alreadyHandled and ran items, computes top-level ok", () => {
  const ran: ExportBatchReportItem[] = [
    { ok: true, slug: "s1", title: "T1", outputDir: "/x", written: ["mp4" as ExportFormat], failed: [], skipped: [] }
  ];
  const alreadyHandled = [
    { ok: true, slug: "s2", title: "T2", outputDir: "/x", written: ["mp4" as ExportFormat], failed: [], skipped: [], resumed: false, reason: "already complete" }
  ];
...
test("buildResumeReport sets ok:false when any ran item failed", () => {
  const ran: ExportBatchReportItem[] = [
    { ok: false, slug: "s1", title: "T1", outputDir: "/x", written: [], failed: [{ format: "mp4" as ExportFormat, error: "boom" }], skipped: [] }
  ];
