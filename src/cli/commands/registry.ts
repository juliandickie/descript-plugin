...
import { parseNamingManifest, renderBatchNames, VIMEO_NAME_CAP, type NamingManifest } from "../../workflows/nameTemplate.js";
...
export const COMMANDS: Record<string, (ctx: Ctx) => Promise<number>> = {
  ...
  async jobs(ctx) {
    ...
    const sub = ctx.args[0];
    if (sub === "list") {
      if (badEnum(ctx, "type", JOB_TYPE)) return 2;
  ...
  async projects(ctx) {
    ...
    const sub = ctx.args[0];
    if (sub === "list") {
      if (badEnum(ctx, "sort", PROJECT_SORT)) return 2;
      if (badEnum(ctx, "direction", PROJECT_DIRECTION)) return 2;
      ...
      const query: ListProjectsQuery = {
        ...
        sort: typeof ctx.flags["sort"] === "string" ? ctx.flags["sort"] as ListProjectsQuery["sort"] : undefined,
        direction: typeof ctx.flags["direction"] === "string" ? ctx.flags["direction"] as ListProjectsQuery["direction"] : undefined,
        limit,
        cursor: typeof ctx.flags["cursor"] === "string" ? ctx.flags["cursor"] : undefined,
      };
  ...
  async export(ctx) {
    ...
    let naming: NamingManifest | undefined;
    if (namingActive) {
      let rawManifest: unknown = {};
      if (namesFlag) {
        rawManifest = readJsonFile(ctx, namesFlag);
        if (rawManifest === undefined) return 2;
      }
      try {
    ...
    let items: Array<{ projectId: string; compositionId: string; projectFolder?: string; title?: string }> = [];
    ...
    let batchItems: ExportBatchItem[] = items.map(({ title: _t, ...rest }) => rest);
    if (naming) {
