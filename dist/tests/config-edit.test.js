...
test("editor precedence: --editor over VISUAL over EDITOR, darwin default is open -t", () => {
    const t = tmpCfg();
    const calls = [];
    const spy = (cmd, args) => { calls.push({ cmd, args }); };
...
test("descript config edit runs end to end and exits 0", async () => {
    ...
    const out = [];
    const code = ce({ flags: { profile: "smoke" }, io: { stdout: (s) => out.push(s), stderr: (s) => out.push(s), json: false }, env: {}, configPath: t.path, spawnEditor: () => { }, platform: "linux" });
    assert.equal(code, 0);
