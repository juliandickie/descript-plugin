...
test("download-published --slugs s1,s2 fans out", async () => {
    ...
    const out = [];
    const code = await runCli(["download-published", "--slugs", "a,b", "--output-dir", dir, "--formats", "mp4", "--concurrency", "1", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 0);
...
test("download-published --report reads slugs from a prior export-report.json", async () => {
    ...
    const out = [];
    const code = await runCli(["download-published", "--report", reportPath, "--output-dir", dir, "--formats", "md", "--concurrency", "1", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 0);
...
test("download-published with two slug sources (positional + --slugs) exits 2", async () => {
    const out = [];
    const code = await runCli(["download-published", "abc", "--slugs", "def"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 2);
...
test("export PID CID publishes and downloads in one go", async () => {
    ...
    const out = [];
    const code = await runCli(["export", "p", "c", "--output-dir", dir, "--formats", "mp4,srt,md", "--access-level", "private", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 0);
...
test("export rejects empty --formats value at parse time (v0.3.0 followup §2.4)", async () => {
    const { calls } = installMockFetch([{ status: 200, json: {} }]);
    const out = [];
    const code = await runCli(["export", "p", "c", "--formats", "", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 2);
    assert.equal(calls.length, 0);
...
test("export then download-published --report round-trips against the same temp dir", async () => {
    ...
    const exportCode = await runCli(["export", "rt-proj", "rt-comp", "--output-dir", dir, "--formats", "md", "--access-level", "private", "--concurrency", "1", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    ...
    const dlCode = await runCli(["download-published", "--report", exportReportPath, "--output-dir", dir, "--formats", "md", "--concurrency", "1", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
...
test("transcript prints text formats to stdout", async () => {
    ...
    const c = capture();
    const code = await runCli(["transcript", "p1", "c1", "--format", "txt", "--speaker-labels", "changes"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    assert.equal(code, 0);
...
test("transcript --json wraps content", async () => {
    ...
    const c = capture();
    const code = await runCli(["transcript", "p1", "--format", "srt", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    assert.equal(code, 0);
    const parsed = JSON.parse(c.out.join(""));
...
test("transcript --out writes the file", async () => {
    ...
    const c = capture();
    const code = await runCli(["transcript", "p1", "--format", "markdown", "--out", out], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    assert.equal(code, 0);
...
test("transcript docx without --out is a usage error before any API call", async () => {
    installNoNetwork();
    const c = capture();
    const code = await runCli(["transcript", "p1", "--format", "docx"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    assert.equal(code, 2);
...
test("transcript timecode flags build the timecodes object", async () => {
    ...
    const c = capture();
    const code = await runCli(["transcript", "p1", "--format", "txt", "--timecodes-every", "30", "--timecodes-on-paragraphs"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    assert.equal(code, 0);
    const body = JSON.parse(calls[0].body);
...
test("transcript --timecodes-on-speakers and --timecodes-on-markers reach the API body", async () => {
    const { calls } = installMockFetch([{ status: 200, text: "x", headers: { "content-type": "text/plain" } }]);
    const c = capture();
    const code = await runCli(["transcript", "p1", "--format", "txt", "--timecodes-on-speakers", "--timecodes-on-markers"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    assert.equal(code, 0);
    const body = JSON.parse(calls[0].body);
...
test("import --workspace maps to workspace_name", async () => {
    const { calls } = installMockFetch([{ status: 201, json: { job_id: "j", drive_id: "d", project_id: "p", project_url: "u" } }]);
    const c = capture();
    const code = await runCli(["import", "--url", "https://x/y.mp4", "--workspace", "General", "--team-access", "view", "--no-wait", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    assert.equal(code, 0);
    const body = JSON.parse(calls[0].body);
...
// ---------------------------------------------------------------- v0.7.0 --names
function writeNamesFile(dir, languages) {
...
test("export --names renders the iDD standard, writes flat, reports renderedName", async () => {
    ...
    const out = [];
    const code = await runCli(["export", "p", "--names", namesPath, "--output-dir", dir, "--formats", "srt", "--concurrency", "1", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 0);
...
test("export --names pre-flight failure exits 2 before any publish", async () => {
    ...
    const out = [];
    const code = await runCli(["export", "p", "--names", namesPath, "--output-dir", dir, "--formats", "srt", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 2);
...
test("export --names cannot combine with --resume", async () => {
    const out = [];
    const code = await runCli(["export", "--resume", "/nonexistent.json", "--names", "/also-nonexistent.json", "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 2);
...
test("export --name-template {slug} is rejected in export mode", async () => {
    ...
    const out = [];
    const code = await runCli(["export", "p", "c", "--name-template", "{slug}", "--output-dir", dir, "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 2);
...
test("export --names duplicate language mapping exits 2 naming both compositions", async () => {
    ...
    const out = [];
    const code = await runCli(["export", "p", "--names", namesPath, "--output-dir", dir, "--json"], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: (s) => out.push(s), stderr: (s) => out.push(s) });
    assert.equal(code, 2);
...
// =========================================================================
async function publishBody(extra) {
    const { calls } = installMockFetch([{ status: 201, json: { job_id: "j1" } }]);
    const c = capture();
    const code = await runCli(["publish", "--project-id", "p1", "--no-wait", "--json", ...extra], { env: { DESCRIPT_API_TOKEN: "t" }, stdout: c.write, stderr: c.write });
    return { code, body: calls[0] ? JSON.parse(calls[0].body) : undefined, calls: calls.length, out: c.out.join("") };
}
