...
test("sends bearer auth and parses JSON", async () => {
    ...
    const res = await http.request("GET", "/status");
...
test("maps error status to DescriptApiError", async () => {
    ...
    const http = new HttpClient({ token: "t" });
    await assert.rejects(() => http.request("GET", "/status"), (e) => e instanceof DescriptApiError && e.status === 401 && e.category === "unauthorized");
});
...
test("gives up after maxRetries on persistent 429", async () => {
    ...
    await assert.rejects(() => http.request("GET", "/status"), (e) => e instanceof DescriptApiError && e.status === 429 && e.retryAfterSeconds === 0);
    assert.equal(calls.length, 3);
});
test("returns undefined for 204 No Content", async () => {
    installMockFetch([{ status: 204, text: "" }]);
    const http = new HttpClient({ token: "t" });
    const res = await http.request("DELETE", "/jobs/j1");
