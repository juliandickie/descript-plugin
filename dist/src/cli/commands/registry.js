...
export const GLOBAL_FLAGS = ["json", "help", "token", "profile"];
...
export const COMMANDS = {
    ...
    async translate(ctx) {
        ...
        for (const c of out.newCompositions)
    ...
    async jobs(ctx) {
        ...
        if ((sub === "get" || sub === "cancel") && !ctx.args[1]) {
            fail(ctx.io, `Usage: descript jobs ${sub} <id>`);
            return 2;
        }
        if (sub === "get") {
    ...
    async projects(ctx) {
        ...
        if (sub === "get") {
            if (!ctx.args[1]) {
                fail(ctx.io, "Usage: descript projects get <id>");
                return 2;
            }
