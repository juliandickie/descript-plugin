 ...
 */
export function validateRequestedFormatsAgainstReport(report, requestedFormats) {
    if (!requestedFormats || requestedFormats.length === 0)
        return { ok: true };
    const attempted = new Set();
    for (const item of report.items) {
        for (const f of item.written)
            attempted.add(f);
        for (const { format } of item.failed)
 ...
 */
function unionAttempted(report) {
    const set = new Set();
    for (const item of report.items) {
        for (const f of item.written)
            set.add(f);
 ...
 */
function fileExistsFor(itemDir, safeTitle, fmt) {
    if (fmt === "mp4") {
 ...
 */
export function reconstructResumeItems(prior, requestedFormats) {
    const itemsToRun = [];
    const alreadyHandled = [];
    // Effective format set for the resume call.
    const effective = requestedFormats && requestedFormats.length > 0
        ? requestedFormats
        : unionAttempted(prior);
    for (const item of prior.items) {
        const itemAttempted = new Set();
        for (const f of item.written)
            itemAttempted.add(f);
        for (const { format } of item.failed)
            itemAttempted.add(format);
        // Per-item effective formats (narrowed to what this item actually attempted).
        const itemEffective = effective.filter((f) => itemAttempted.has(f));
        // partially_resumable when the requested set asks for formats this item never attempted.
        const partiallyResumable = effective.length !== itemEffective.length;
        ...
        if (itemEffective.length === 0) {
            alreadyHandled.push({
                ...
                ...(partiallyResumable ? { partially_resumable: true } : {})
            });
            continue;
        }
        // Decide per-format whether to redo.
        const safeTitle = sanitize(item.title || "untitled");
        const failedFmts = new Set(item.failed.map((f) => f.format));
        const formatsToRedo = [];
        for (const f of itemEffective) {
            ...
            if (wasFailed) {
                ...
                if (!fileExistsFor(item.outputDir, safeTitle, f))
                    formatsToRedo.push(f);
            }
        ...
        const skipFormats = effective.filter((f) => !formatsToRedo.includes(f));
 ...
 */
export function buildResumeReport(resumedFrom, batchItems, alreadyHandled, allSkipped) {
    // Mark each batch-produced item as resumed: true.
    const ranItems = batchItems.map((i) => ({
        ...i,
        resumed: true
    }));
    ...
    // correctness.
    const items = [...alreadyHandled, ...ranItems];
