...
export async function pollJob(getJob, jobId, opts = {}) {
    ...
    for (;;) {
        ...
        try {
            ...
            const transient = (e instanceof DescriptApiError && e.status >= 500) ||
                !(e instanceof DescriptApiError);
            if (transient && pollAttempt < maxPollRetries) {
                pollAttempt += 1;
                if (now() - start >= maxWait) {
                    throw new Error(`Polling timed out after ${maxWait}ms for job ${jobId} (transient error: ${e instanceof Error ? e.message : String(e)})`);
                }
